...
const COW_ABI = require('./abis/hybird_cow.json');
...
export class HybirdCow {
	...
	async stake(sender, amount, callback) {
	  ...
	  var tx = this.contract.methods.stake();
	  let gasLimit = 550000;

	  try {
	  	gasLimit = await tx.estimateGas({ value: weiAmount, from: sender, to: this.address });
	  } catch(err) {
	  }
	  return tx.send({
	    from: sender,
	...
	async getWallet(user) {
		return await this.contractReader.methods.getWallet(user).call();
	}
}

export default HybirdCow;